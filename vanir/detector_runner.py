  ...
  <body>
    ...
    <table>
      ...
      <tr>
        <th>Ignored CVEs</th>
        <td>
        {{ '<pre><b>Ignored CVEs by ID filter</b></pre>' if ignored_cves['ignored_by_id_filter']|length > 0 else '' }}
...
def _get_cves_from_findings(
    findings: scanner_base.Findings, vuln_manager: vulnerability_manager
) -> Set[str]:
...
def main(argv: Sequence[str]) -> None:
  ...
  for finding_filter in finding_filters:
    ...
    if isinstance(finding_filter, scanner_base.PathPrefixFilter):
      ...
      remaining_cve_ids = _get_cves_from_findings(findings_filtered, vuln_manager)
      ignored_cve_ids = list(all_findings_cve_ids - remaining_cve_ids)
      if ignored_cve_ids:
        ignored_paths[finding_filter._prefix].extend(ignored_cve_ids)

  # sort CVE IDs in ignored paths dict
  ignored_paths = {path: sorted(items) for path, items in ignored_paths.items()}
  ...
  def get_cve_id(vuln_id):
    cve_ids = vuln_manager.osv_id_to_cve_ids(vuln_id)
    return cve_ids[0] if cve_ids else vuln_id

  # Collect CVE IDs filtered out
  filtered_vuln_ids = [
    get_cve_id(vuln.id) for vuln in vuln_manager.get_vulnerabilities(ignore_filters=False)
  ]
  unfiltered_vuln_ids = [
    get_cve_id(vuln.id) for vuln in vuln_manager.get_vulnerabilities(ignore_filters=True)
  ]
  ignored_cve_ids = set(unfiltered_vuln_ids) - set(filtered_vuln_ids)

  # Collect CVE IDs filtered out by VULN_ID_IGNORE_LIST
  ignore_list = [
    alias
    for entry in detector_common_flags._VULN_ID_IGNORE_LIST.value
    for alias in vuln_manager.osv_id_to_cve_ids(entry) + [entry]
  ]
  ignored_by_id_filter = sorted(ignored_cve_ids & set(ignore_list))

  ignored_cves = {}
  if ignored_by_id_filter:
    ignored_cves['ignored_by_id_filter'] = ignored_by_id_filter
  if ignored_paths:
